...
export default function CategoryProductsPage() {
  ...
  // fetch first page of ids when category or selectedSubcategory changes
  useEffect(() => {
    let mounted = true;
    // auto-select first subcategory if available when category loads
    if (category && Array.isArray(category.subcategories) && category.subcategories.length && !selectedSubcategory) {
      const first = category.subcategories[0];
      const firstId = first?.id || first?.slug || first?.key || first;
      if (firstId) setSelectedSubcategory(firstId);
    }

    async function fetchFirst(){
      const activeId = selectedSubcategory || (category && category.id);
      if(!activeId) return;
      setLoading(true);
      try{
        const q = query(ref(db, `/categoryProducts/${activeId}`), orderByKey(), limitToFirst(PAGE_SIZE));
  ...
  const loadMore = async ()=>{
    ...
    setLoadingMore(true);
    try{
      const q = query(ref(db, `/categoryProducts/${activeId}`), orderByKey(), startAt(lastKey), limitToFirst(PAGE_SIZE+1));
      const snap = await get(q);
      const val = snap.exists() ? snap.val() : null;
      const ids = val ? Object.keys(val) : [];
      let newIds = ids;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
      ...
      <div className="text-center space-y-3">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">{category.title || category.displayName || category.name}</h1>
        {category.description && (
          <p className="text-gray-600 max-w-2xl mx-auto">{category.description}</p>
        )}
        {Array.isArray(category?.subcategories) && category.subcategories.length > 0 && (
          <div className="flex gap-3 justify-center mt-3 overflow-x-auto no-scrollbar px-2">
            {category.subcategories.map((s) => {
              const sid = s?.id || s?.slug || s?.key || s;
              const sname = s?.name || s?.title || String(sid);
              return (
                <button key={sid} onClick={() => setSelectedSubcategory(sid)} className={`px-3 py-1 rounded ${selectedSubcategory === sid ? 'bg-orange-50 text-orange-600 font-semibold' : 'bg-white text-gray-700 border border-gray-100'}`}>
                  {sname}
                </button>
              );
            })}
