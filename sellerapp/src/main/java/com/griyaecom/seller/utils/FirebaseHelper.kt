package com.griyaecom.seller.utils
...
/**
 * Firebase initialization helper with robust error handling for Seller App
 */
object FirebaseHelper {
    private const val TAG = "SellerFirebaseHelper"
    ...
    /**
     * Subscribe to seller-specific topics with error handling
     */
    suspend fun subscribeToSellerTopics(
    ...
    /**
     * Update seller status with topic management
     */
    suspend fun updateSellerStatus(
        ...
        try {
            when (status) {
                "open" -> {
                    subscribeToTopics(
                        listOf("order_notifications", "payment_notifications"),
                        onSuccess = { Log.d(TAG, "Subscribed to $it for open status") }
                    )
                }

                "closed" -> {
                    unsubscribeFromTopics(
                        listOf("order_notifications"),
                        onSuccess = { Log.d(TAG, "Unsubscribed from $it for closed status") }
                    )
                    // Keep payment notifications even when closed
                }

                "busy" -> {
                    // Maybe unsubscribe from new order notifications temporarily
                    Log.d(TAG, "Status updated to busy")
                }
            }
            onSuccess()
        } catch (e: Exception) {
            Log.e(TAG, "Failed to update seller status: $status", e)
            onError(e)
        }
    }
    ...
    private fun storeTokenLocally(context: Context, token: String) {
        try {
            val sharedPref = context.getSharedPreferences("SellerFCMPrefs", Context.MODE_PRIVATE)
            sharedPref.edit()
