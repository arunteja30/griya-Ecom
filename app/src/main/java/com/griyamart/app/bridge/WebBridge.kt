...
class WebBridge(
        ...
        notificationHelper.getFCMToken { token ->
            activity.runOnUiThread {
                val result = JSONObject().apply {
                    ...
                    put("token", token ?: "")
                    put("message", if (token != null) "Token registered successfully" else "Failed to get FCM token")
                }
        ...
        notificationHelper.unsubscribeFromTopic(topic) { success ->
            activity.runOnUiThread {
                val result = JSONObject().apply {
                    put("success", success)
                    put("topic", topic)
                    put("message", if (success) "Unsubscribed from topic" else "Failed to unsubscribe")
                }
                val jsCode = "window.onTopicUnsubscription && window.onTopicUnsubscription(${result})"
                activity.getWebView().evaluateJavascript(jsCode, null)
            }
        }
    }
    ...
    fun goForward() {
        activity.runOnUiThread {
            if (activity.getWebView().canGoForward()) {
                activity.getWebView().goForward()
            }
        }
    }
