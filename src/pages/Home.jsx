          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(banners).map(([key, banner]) => (
              <div key={key} className="relative h-64 bg-gray-100 rounded overflow-hidden">
                <UniversalImage src={banner.image || ''} alt={banner.title || 'Banner'} className="w-full h-full object-cover" fallback={'/placeholder.jpg'} />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(products).map(([id, p]) => (
              <Link to={`/products/${id}`} key={id} className="block border rounded p-2">
                <UniversalImage src={p.image || ''} alt={p.title} className="w-full h-40 object-cover mb-2 rounded" fallback={'/placeholder.jpg'} />
