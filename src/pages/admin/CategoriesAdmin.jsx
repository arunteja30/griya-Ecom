    ...
    <div>
      ...
      <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-3 items-start">
        ...
        <div className="md:col-span-2 flex items-center justify-end gap-3">
          ...
          {editing && (
            <button 
              onClick={() => { setEditing(null); setCategoryId(''); setName(''); setSlug(''); setImageUrl(''); setDescription(''); setSortOrder(0); setTags([]); }} 
            ...
            <div key={id} className="border p-3 rounded bg-white">
              ...
              <div className="flex items-center justify-end gap-2 mt-3">
                {editing === id ? (
                  ...
                  <>
                    <button onClick={()=>{setEditing(id); setCategoryId(id); setName(cat.name); setSlug(cat.slug||''); setImageUrl(cat.image||''); setDescription(cat.description||''); setSortOrder(cat.sortOrder||0); setTags(cat.tags||[]);}} className="text-blue-600 text-sm">Edit</button>
              ...
              <div className="flex gap-2">
                {editing === id ? (
                  <>
                    ...
                    <button onClick={() => { setEditing(null); setCategoryId(''); setName(''); setSlug(''); setImageUrl(''); setDescription(''); setSortOrder(0); setTags([]); }} className="text-gray-600">Cancel</button>
                  </>
                ) : (
                  <>
                    <button onClick={()=>{setEditing(id); setCategoryId(id); setName(cat.name); setSlug(cat.slug||''); setImageUrl(cat.image||''); setDescription(cat.description||''); setSortOrder(cat.sortOrder||0); setTags(cat.tags||[]);}} className="text-blue-600">Edit</button>
