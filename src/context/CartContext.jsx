...
export function CartProvider({ children }) {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          const normalized = parsed.map((item) => {
            ...
            if (item.id && item.product) return { ...item, quantity: Math.max(1, Math.floor(Number(item.quantity) || 1)) };
            ...
            if (item.product) {
              ...
              return { id: pid, product: { ...item.product, id: pid }, quantity: Math.max(1, Math.floor(Number(item.quantity) || 1)) };
            }
            ...
            return { id: pid, product: { ...item, id: pid }, quantity: Math.max(1, Math.floor(Number(item.quantity) || 1)) };
          }).filter(Boolean);
  ...
  const addToCart = (product, qty = 1) => {
    ...
    if (!pid) return;
    // Prefer variant-level stock/quantity if present (product may carry variant in .variant)
    const available = Number(product?.variant?.stock ?? product?.variant?.quantity ?? product?.stock ?? product?.quantity ?? 0) || 0;
    const qtyInt = Math.max(1, Math.floor(Number(qty) || 1));
    setCartItems((prev) => {
      const existing = prev.find((i) => i.id === pid);
      if (existing) {
        const requested = existing.quantity + qtyInt;
      ...
      // New item
      const toAdd = available > 0 ? Math.min(qtyInt, available) : qtyInt;
      if (available > 0 && qtyInt > available) showToast(`Only ${available} items available. Added ${toAdd} to cart.`, 'warning');
  ...
  const updateQuantity = (productId, quantity) => {
    setCartItems((prev) => {
      ...
      // Existing.product may include a variant object — prefer that
      const available = Number(existing.product?.variant?.stock ?? existing.product?.variant?.quantity ?? existing.product?.stock ?? existing.product?.quantity ?? 0) || 0;
